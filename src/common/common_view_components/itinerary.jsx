...
const Itinerary = ({
  ...
  }, [isEditTour, isEditTrekking, isEditPeakClimbing, isEditExpedition, value]);
  ...
  const handleItineraryFactorsUpdate = (
    ...
    setSelectedItineraries((prev) => [...prev, field]);
    const updatedItineraries = itineraries.map((item, idx) => {
      if (itineraryIdx === idx) {
          ...
          itineraries.map((item, idx) => (
            <div key={idx}>
                          ...
                          <div className="space-x-5 space-y-4 w-full flex items-center ">
                            ...
                            >
                              ...
                              {ItineraryFactors?.map((opt, idx) => (
                                <option 
                                disabled={item.itineraryFactors.some(i => i.title == opt)}
                                key={idx} value={opt}>
                                  {opt}
                                </option>
                              ))}
                            ...
                            <button
                              className="bg-red-200 text-red-600 px-2 py-1 rounded hover:bg-red-300 h-[30px] -mt-5 text-xl"
                              onClick={() =>
              ...
              {itineraries.length > 1 && (
                <hr className="opacity-50 py-2 w-[95%]" />
              )}
            </div>
          ))
        ) : (
